...
class Previsor:
    ...
    def carregarModelo(self):
        ...
        for modelo in tqdm.tqdm(resultados_formatados['resultados'].keys(), desc="Carregando modelos"):
            caminho_modelo = constantes.algoritimos_dir + modelo + '.pickle'
    ...
    def adicionarPredicoesAoDataFrame(self):
        for nome_modelo, modelo in tqdm.tqdm(self.modelos.items(), desc="Executando previsões"):
            ...
            self.df[constantes.predicao] = modelo.predict(self.X)
            if hasattr(modelo, 'predict_proba'):
                self.df[constantes.score] = modelo.predict_proba(self.X)[:, 1]
    ...
    def analise(selv):
        ...
        with open(constantes.algoritimos_dir + constantes.resultado_completo_df, 'rb') as file:
